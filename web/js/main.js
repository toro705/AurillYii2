(function($) {
	...
	$(document).on('ready', function() {
		...
		$('#distribuidores .panel-group a').click(function(){
			var href = $(this).attr('href');
			var hrefmap = '#map a[href^="'+href+'"]';

			$('#map a').removeClass('active');
			$(hrefmap).addClass('active');
		});
                $(window).on('load', function() {
                    var secHash = window.location.hash;
			// console.log(secHash);
			switch (secHash) {
				case '#reparar':				
					$('.Reparar').click();
                                        // console.log('reparemoo');
					break;
				case '#balancear':
                                        $('.Balancear').click();
					break;
...
function isElementVisible(a){var b=$(window).scrollTop(),c=b+$(window).height(),d=a.offset().top,e=d+a.height();return e<=c&&d>=b}function animateCounter(a){$(a).prop("Counter",0).animate({Counter:$(a).data("counter-val")},{duration:2e3,easing:"swing",step:function(b){$(a).text(Math.ceil(b))}})}function isDeviceMobile(){return"none"===$("#isMobile").css("display")}!function(){if(window.console){var a=0;a||(setTimeout(function(){},1),a=1)}}();
$(window).bind("scroll",function(){$(window).scrollTop()>300?($(".nav-back").addClass("fixed"),$(".nav-back.fixed").css({"-webkit-animation":"menu-fixed-animation 1000ms",animation:"menu-fixed-a@O     @O                     ∞È›              çˇ     ®O             `O     ∫       `O             -webkit-animation":"menu-animation 1000ms",animation:"menu-animation 1000ms"}))});